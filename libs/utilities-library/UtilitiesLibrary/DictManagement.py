...
class DictManagement:
    ...
    def verify_response_body(self, actual, expect, ignores=None, nonignores=None, check_key=True, check_datatype=True,
                             check_value=True):
        ...
        *Options*
        ...
        """
        diffs = self.get_dict_diffs(actual, expect, ignores, nonignores, check_key, check_datatype, check_value)
        if diffs:
            raise AssertionError(self.print_friendly_message(diffs))

    def get_dict_diffs(self, actual, expect, ignores=None, nonignores=None, check_key=True, check_datatype=True,
                       check_value=True):
        """
        ...
        if check_value:
            if ignores and isinstance(ignores, list):
                ...
                if check_key:
                    key_diffs = list(diff(expect, actual))
                    key_diffs = list(d for d in key_diffs if (d[0] == 'add' or d[0] == 'remove'))
                del_expected = self.update_value_from_original_dictionary(expect, nonignores)
                del_actual = self.update_value_from_original_dictionary(actual, nonignores)
    ...
    def update_data_into_original_dictionary(self, original_source, overrides):
        """
        ...
        if (overrides is not None) and (overrides != ''):
            result = self.recursive_update_data_into_original_dictionary(source, overrides)
        else:
            result = source
        return result


    def recursive_update_data_into_original_dictionary(self, source, overrides):
        """
        ...
        for key, value in overrides.items():
            if isinstance(value, dict):
                self.recursive_update_data_into_original_dictionary(source.get(key, {}), value)
            elif isinstance(value, list):
                for i in range(len(value)):
                    self.recursive_update_data_into_original_dictionary(source.get(key)[i], value[i])
            else:
    ...
    def convert_suds_object_to_dict(self, my_suds):
        """Converts a suds object to dictionary."""
        logger.debug("Before converting:\n")
        logger.debug(my_suds)
        result = self.recursive_convert_suds_object_to_dict(my_suds)
        logger.debug("After converting:\n")
        logger.debug(result)
        return result


    def recursive_convert_suds_object_to_dict(self, obj):
        ...
        for field in fields:
            ...
            if isinstance(val, list):
                data[field] = []
                for item in val:
                    data[field].append(self.recursive_convert_suds_object_to_dict(item))
            else:
                data[field] = self.recursive_convert_suds_object_to_dict(val)
        return data


    @keyword(name="Update The Value For The Key List", tags=(TAG,))
    def update_the_value_for_the_key_list(self, mydic, keys, values):
        """
        ...
        try:
            i = 0
            for key in keys:
                mydic = self.update_the_value_for_selected_key(mydic, key, values[i])
                i += 1
            return mydic
    ...
    def update_the_value_for_selected_key(self, mydic, keyid, valueid):
        ...
        result = self.recursive_update_the_value_for_selected_key(source, keyid, valueid)
        return result

    def recursive_update_the_value_for_selected_key(self, mydic, keyid, valueid):
        """
        ...
        try:
            for key, value in mydic.items():
                if isinstance(value, dict):
                    self.recursive_update_the_value_for_selected_key(value, keyid, valueid)
                elif isinstance(value, list):
                    for i in range(len(value)):
                        self.recursive_update_the_value_for_selected_key(value[i], keyid, valueid)
                elif key == keyid:
                    mydic[key] = valueid
            return mydic
        except Exception as e:
    ...
    def get_the_value_for_selected_key(self, mydict, keyid):
        """
            ...
            if isinstance(mydict, dict):
                for a_k, a_v in mydict.items():
                    if isinstance(a_v, dict):
                        myvalue = self.get_the_value_for_selected_key(a_v, keyid)
                    elif isinstance(a_v, list):
                        for i in range(len(a_v)):
                            myvalue = self.get_the_value_for_selected_key(a_v[i], keyid)
                    else:
                        if a_k == keyid:
            ...
            if isinstance(mydict, dict):
                for k, v in mydict.items():
                    if isinstance(v, dict):
                        yield k
                        yield from self.get_keys(v)
                    elif isinstance(v, list):
                        for i in v:
                            yield from self.get_keys(i)
                    else:
                        yield k
            elif isinstance(mydict, list):
                for i in mydict:
    ...
    def get_list_diffs(self, list1, list2):
        """
    ...
    def delete_key_in_dictionary(self, original_source, key):
        ...
        """
        source = copy.deepcopy(original_source)
        result = self.recursive_delete_key_in_dictionary(source, key)
        return result


    def recursive_delete_key_in_dictionary(self, source, key_delete):
        """
        ...
        for key, value in source.items():
            if isinstance(value, dict):
                self.recursive_delete_key_in_dictionary(value, key_delete)
    ...
    def update_value_from_original_dictionary(self, source, source_target):
        ...
        try:
            ...
            logger.debug("Target:\n")
            logger.debug(target)
            self.check_type(source, target)
            result = self.recursive_update_value_from_original_dictionary(source, target)
            logger.debug("Result:\n")
            logger.debug(result)
            return result
        except Exception as e:
    ...
    def recursive_update_value_from_original_dictionary(self, source, target):
        """
        ...
        try:
            for key, value in target.items():
                if isinstance(value, dict):
                    self.recursive_update_value_from_original_dictionary(source.get(key, {}), value)
                elif isinstance(value, list):
                    for i in range(len(value)):
                        if not isinstance(value[i], str):
                            self.recursive_update_value_from_original_dictionary(source.get(key)[i], value[i])
                        else:
    ...
    def update_the_value_for_the_key_list_to_decimal(self, mydict, keys):
        """
        ...
        try:
            for key in keys:
                mydict = self.update_the_value_for_selected_key_to_decimal(mydict, key)
            return mydict
        except Exception as e:
    ...
    def update_the_value_for_selected_key_to_decimal(self, mydic, keyid):
        """
        ...
        """
        source = copy.deepcopy(mydic)
        result = self.recursive_update_the_value_for_selected_key_to_decimal(source, keyid)
        return result


    def recursive_update_the_value_for_selected_key_to_decimal(self, mydict, keyid):
        """
        ...
        try:
            ...
            if isinstance(mydict, dict):
                for a_k, a_v in mydict.items():
                    if isinstance(a_v, dict):
                        myvalue = self.recursive_update_the_value_for_selected_key_to_decimal(a_v, keyid)
                    elif isinstance(a_v, list):
                        for i in range(len(a_v)):
                            myvalue = self.recursive_update_the_value_for_selected_key_to_decimal(a_v[i], keyid)
                    else:
                        if a_k == keyid:
                            myvalue = mydict[keyid]
    ...
    def validate_response_body(self, actual, expect, ignores=None, nonignores=None, check_key=True, check_datatype=True,
                               check_value=True):
        ...
        *Options*
        ...
        diffs = self.get_diffs(actual, expect, ignores, nonignores, check_key, check_datatype, check_value)
        if diffs:
            raise AssertionError(self.print_friendly_message(diffs))


    def get_diffs(self, actual, expect, ignores=None, nonignores=None, check_key=True, check_datatype=True, check_value=True):
        """
        Compare the value between two dictionaries and return the change between two dictionaries.
