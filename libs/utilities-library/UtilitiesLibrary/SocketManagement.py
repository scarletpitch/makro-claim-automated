...
class SocketManagement:
    ...
    def encrypt_sha256(self, text: str, salt: str = "", encode: str = UTF_8) -> str:
        """
        ...
        return self.encode_base64(bytes(text))


    @keyword(name="Send Message Socket", tags=(TAG,))
    def send_message_socket(self, host: str, port: int, message: bytes) -> bytes:
        """
        ...
        """
        text_byte = self.decode_base64(message)
    ...
    def update_message_socket(self, text_base64: bytes, message: str, offset: int, length: int,
        ...
        if message is None:
            message = ''
        if encode == self.TIS_620:
            text_byte = self.convert_str_to_bytes(message, length, encode)
        else:
            text_byte = self.convert_int_to_hex(message, length)
        text_decode = self.decode_base64(text_base64)
        new_message = text_decode[:offset] + text_byte + text_decode[offset + length:]
        return self.encode_base64(new_message)


    def convert_str_to_bytes(self, text: str, length: int, encode: str) -> bytes:
        """
        Convert string to Bytes.
        """
        space = self.make_pattern(text, length, self.SPACE)
        return bytes(text + space, encode)


    def convert_int_to_hex(self,text: str, length: int) -> bytes:
        """
        Convert Integer to Hex.
        """
        number = hex(int(text))[2:]
        zero = self.make_pattern(number, 2 * length, self.ZERO)
        array = bytearray.fromhex(zero + number)
        return bytes(array)


    def make_pattern(self,text: str, length: int, pattern: str):
        """
        Make pattern.
        """
    ...
    def convert_hex_to_int(self,number: bytes) -> int:
        """
    ...
    def encode_base64(self,text: bytes) -> bytes:
        """
    ...
    def decode_base64(self,text: bytes) -> bytes:
        """
